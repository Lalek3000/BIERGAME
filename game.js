...
function startMenuMusic(){
    if(!menuMusic&&(st==="menu"||st==="name-input"||st==="rules")){
        try {
            menuMusic=new Audio("backgroundmenu.mp3"); menuMusic.loop=true; menuMusic.volume=0.4;
            menuMusic.play().catch(e=>{});
        } catch(e) { menuMusic = null; }
    }
}
function startMusic(){
    if(menuMusic){try{menuMusic.pause();}catch(e){} menuMusic=null;}
    try {
        if(!bgMusic){bgMusic=new Audio("background.mp3"); bgMusic.loop=true; bgMusic.volume=0.4;}
        if(!gitoMusic){gitoMusic=new Audio("backgroundgito.mp3"); gitoMusic.loop=true; gitoMusic.volume=0.45;}
        switchTrack("normal");
    } catch(e) { bgMusic = null; gitoMusic = null; }
}
function switchTrack(t){
    try{
        if(t==="gito"&&gitoMusic&&bgMusic){bgMusic.pause(); gitoMusic.play().catch(e=>{});}
        if(t==="normal"&&bgMusic&&gitoMusic){gitoMusic.pause(); bgMusic.play().catch(e=>{});}
    }catch(e){}
}
...
const p2=new P(330,500,"#00ffcc",{u:"KeyT",d:"KeyG",l:"KeyF",r:"KeyH",a:"KeyR"});
